...
import { PlatformVMConstants, StakeableLockOut } from 'luxnet/dist/apis/platformvm'
...
const assets_module: Module<AssetsState, RootState> = {
    ...
    actions: {
        ...
        async addUnknownAsset({ state, commit }, assetId: string) {
            ...
            const newAsset = new LuxAsset(assetId, desc.name, desc.symbol, desc.denomination)
        ...
        async addUnknownNftFamily({ state, commit }, assetId: string) {
            const desc = await ava.XChain().getAssetDescription(assetId)
            const newFam = new LuxNftFamily(assetId, desc.name, desc.symbol)
    ...
    getters: {
        ...
        walletPlatformBalance(
            ...
            if (!wallet || !state.LUX_ASSET_ID) return balances
            ...
            const utxos = utxoSet.getAllUTXOs()
            // Only use LUX UTXOs
            const luxID = bintools.cb58Decode(state.LUX_ASSET_ID)
        ...
        AssetLUX(state, getters, rootState, rootGetters): LuxAsset | null {
            ...
            const LUX_ASSET_ID = state.LUX_ASSET_ID
            if (LUX_ASSET_ID) {
