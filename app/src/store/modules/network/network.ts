...
import { getConfigFromUrl, setNetworkAsync } from '@luxdefi/luxnet-wallet-sdk'
    ...
    actions: {
        ...
        async loadSelectedNetwork({ dispatch, getters }): Promise<boolean> {
            ...
            try {
                ...
                const nets: LuxNetwork[] = getters.allNetworks
        ...
        load({ dispatch }) {
            ...
            if (data) {
                const networks: LuxNetwork[] = JSON.parse(data)

                networks.forEach((n) => {
                    const newCustom = new LuxNetwork(
                        n.name,
                        n.url,
                        //@ts-ignore
