...
import { Credential, SigIdx, Signature, UTXOResponse, Address } from 'luxnet/dist/common'
...
class LedgerWallet extends AbstractHdWallet implements LuxWalletCore {
    ...
    static async fromTransport(t: Transport) {
        ...
        const xpub = await prov.getXPUB(t, LUX_ACCOUNT_PATH)
        const hd = new HDKey()
    ...
    async verifyAddress(index: number, internal = false, chainAlias?: ChainIdType) {
        const hrp = luxnet.getHRP()
        const change = internal ? '1' : '0'
        const path = `${LUX_ACCOUNT_PATH}/${change}/${index}`
