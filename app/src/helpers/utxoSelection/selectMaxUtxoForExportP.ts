...
/**
 * Return the maximum number of UTXOs that can be consumed for this transaction, and their LUX sum.
 * @param utxos Luxilable UTXOs in preferred order.
 */
export function selectMaxUtxoForExportP(utxos: UTXO[]) {
    ...
    const finalLuxAmount = sumUtxos(finalUtxos)

    return {
        amount: finalLuxAmount,
        utxos: finalUtxos,
    }
}
